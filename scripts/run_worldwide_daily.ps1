...
Invoke-JobIntel --db $Database collect `
    --sources $Sources `
    --evidence-dir $PublicEvidence `
    --output $Workbook `
    --no-export

if (-not $SkipGmail -and (Test-Path $GmailCredentials)) {
    Write-Host "[2/4] Importing authorized Gmail job alerts..."
    Invoke-JobIntel --db $Database import-gmail `
        --credentials $GmailCredentials `
        --token $GmailToken `
        --max-results $GmailMaxResults `
        --evidence-dir $PrivateEvidence `
        --output $Workbook `
        --no-export
} else {
...
Invoke-JobIntel --db $Database export --output $Workbook

Write-Host "[4/4] Verifying database, evidence and workbook..."
Invoke-JobIntel --db $Database verify --output $Workbook
