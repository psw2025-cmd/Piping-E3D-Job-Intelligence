...
def main()->int:
 ...
 if not robots.can_fetch(UA,LISTING):raise SystemExit('robots.txt does not permit catalogue retrieval')
 jobs={};facets={};links=set()
 for url in LISTING_VARIANTS:
  if not robots.can_fetch(UA,url):continue
  try:
 ...
 for sm in (f'{BASE}/sitemap.xml',f'{BASE}/robots.txt'):
  if not robots.can_fetch(UA,sm):continue
  try:
   r=get(s,sm,a.timeout)
   if sm.endswith('sitemap.xml'):links.update(re.findall(r'https://careers\.fluor\.com/careers/job/[^<\s]+',r.text))
 ...
 for i,job in enumerate(jobs.values()):
  ...
  if i<a.max_details and urlparse(url).hostname=='careers.fluor.com' and robots.can_fetch(UA,url):
   try:
