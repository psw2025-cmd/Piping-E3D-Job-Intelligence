...
def _card_context(anchor, include_terms: tuple[str, ...]) -> str:
    element = anchor
...
def _extract_cards(
    ...
    for anchor in soup.find_all("a", href=True):
        ...
        context = _card_context(anchor, include_terms)
        ...
        if exclude_terms and _matches_any(context, exclude_terms):
            continue
        seen.add(target)
        output.append(_CardLink(target, link_text, context))
    return output
...
def _guess_title(card: _CardLink) -> str:
    ...
    for line in lines:
        ...
        if (
            lowered not in _GENERIC_LINK_TEXT
            and "closing date" not in lowered
            and "apply now" not in lowered
            and len(line) <= 180
        ):
            return line
    return ""
